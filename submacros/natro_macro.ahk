...
nm_HotbarWhile()
MainGui.Add("Button", "x200 y109 w90 h30 vManualHotbarButton Disabled", "Manual Hotbar`n(" ManualHotbarHotkey ")").OnEvent("Click", nm_showManualHotbar)
...
MainGui.Add("Button", "x305 yp+17 w180 h16 vStickerPrinter Disabled", "Sticker Printer Settings").OnEvent("Click", nm_StickerPrinterGui)
...
nm_TabBoostLock(){
	...
	MainGui["StickerPrinter"].Enabled := 0
	MainGui["ManualHotbarButton"].Enabled :=0
}
nm_TabBoostUnLock(){
	global
	...
	MainGui["StickerPrinter"].Enabled := 1
	MainGui["ManualHotbarButton"].Enabled := 1
}
nm_TabQuestsLock(){
	global
...
nm_PrinterTimer(GuiCtrl, *){
	...
	switch StickerPrinter["PrinterTimer"].Text, 0
	{
		...
		StickerPrinter["PrinterTime"].Enabled := 1
		default:
		i := 1
		StickerPrinter["PrinterTimeEdit"].Enabled := 0
		StickerPrinter["PrinterTime"].Enabled := 0
	}
...
nm_StickerPrinterGui(*) {
	...
	StickerPrinter:=Gui("-MinimizeBox +Owner" MainGui.Hwnd, "Sticker Printer")
	StickerPrinter.Show("x" gx+80 " y" gy+35 " w140 h100")
	loop 2 {
		xCoords := 13 + (A_Index=2 ? 68 : 0)
		StickerPrinter.Add("Button", "x" xCoords+2 " y74 w40 h15 vPrinterAdd" A_index, (PrinterItem%A_index% = "None") ? "Add" : "Clear").OnEvent("Click", nm_setPrinterData)
		StickerPrinter.Add("Text", "x" 53 + (A_Index=2 ? 62 : 0) " y5 w27 +Center vPrinterData" A_index, "[" ((PrinterAmount%A_index% = "Infinite") ? "∞" : PrinterAmount%A_index%) "]")
		...
		StickerPrinter.Add("Text", "x" xCoords-2 " y90 w" 40+8 " +Center vPrinterTimer" A_Index, PrinterItem%A_Index% != "None" ? formathm(PrinterTimer%A_Index%) : "00h 00m")
	}
	PrinterAddIndex := 0
	StickerPrinter.Add("Text", "x145 y8 w35 h15 vPrinterTimer +Center +BackgroundTrans Hidden Disabled")
	...
	StickerPrinter.Add("CheckBox", "xp+28 yp+23 w70 h13 vPrinterOption Disabled Hidden", "Infinite").OnEvent("Click", nm_PrinterIndexOption)
	SendMessage(0x1501, 1, StrPtr("Count"),
	StickerPrinter.Add("Edit", "xp yp+15 w70 h18 limit3 Number vPrinterInt Disabled Hidden"))
	StickerPrinter.Add("UpDown", "vPrinterAmount range1-999 Disabled Hidden")
	...
	StickerPrinter.Add("Text", "x128 y27 w70 vPrinterTimeEdit Center Hidden")
	StickerPrinter.Add("UpDown", "vPrinterTime yp-1 h16 range6-9999 Disabled Hidden", 6).OnEvent("Change", (*) => StickerPRinter["PrinterTimeEdit"].text := formathm(StickerPrinter["PrinterTime"].Value * 600))
	StickerPrinter.Add("Button", "vPrinterHelp xp-20 y70 w15 h15 +Center Disabled Hidden", "?").OnEvent("Click", (*)=>MsgBox("There are three options to select.`nYou can change the egg with the arrows on either side of the current selected egg.`n`nYou can change the how often it goes to the printer with the top right control. Manual allows you to set the hourly time while daily will do every 24 hours.`n`nFinnally you can change how many eggs to donate before stopping."))
}
formathm(int) {
	h := Floor(int / 3600)
	m := Floor((int - h * 3600) / 60)
	return (h < 10 ? "0" h : h) "h " (m < 10 ? "0" m : m) "m"
}
nm_PrinterIndexOption(*) {
	global
	PrinterOption := StickerPrinter["PrinterOption"].Value
	if (PrinterOption)
		StickerPrinter["PrinterAmount"].Enabled:=0, StickerPrinter["PrinterInt"].Enabled:=0
	else
		StickerPrinter["PrinterAmount"].Enabled:=1, StickerPrinter["PrinterInt"].Enabled:=1
}
nm_setPrinterData(GuiCtrl, *){
	global
	local i := SubStr(GuiCtrl.Name, -1), sx,sy
	StickerPRinter.getPos(&sx,&sy)
	StickerPrinter.move(sx,sy,(PrinterItem%i% = "None" ? [210,124] : [156, 139])*)
	if (PrinterItem%i% = "None") {
		PrinterAddIndex := i
		loop 2 {
			...
			StickerPrinter["PrinterTimer" A_Index].Visible := 0
		}
		StickerPrinter["StickerPrinterEgg"].Value := ""
		...
		for v in ["PrinterOption", "PrinterInt", "PrinterAmount", "PrinterAddSlot", "StickerPrinterEgg", "SPELeft", "SPERight", "OptLeft", "OptRight", "PrinterTimer", "PrinterTimeEdit", "PrinterTime", "PrinterHelp"]
			StickerPrinter[v].Enabled := 1, StickerPrinter[v].Visible := 1
	} else {
		PrinterItem%i% := "None", PrinterAmount%i% := 0
		IniWrite "None", "settings\nm_config.ini", "Collect", "PrinterItem" i
		IniWrite 0, "settings\nm_config.ini", "Collect", "PrinterAmount" i
		StickerPrinter["PrinterAdd" i].Text := ((PrinterItem%i% = "None" || PrinterItem%i% = "") ? "Add" : "Clear")
		StickerPrinter["PrinterData" i].Text := "[" ((PrinterAmount%i% = "Infinite") ? "∞" : PrinterAmount%i%) "]"
		StickerPrinter["StickerPrinterEgg" i].Value := ""
		StickerPrinter["PrinterTimer" i].Text := "00h 00m"
	}
}
nm_AddPrinterItem(*) {
	Global
	local PrinterOption := StickerPrinter["PrinterOption"].Value, i:=PrinterAddIndex, sx,sy
	if !StickerPrinter["PrinterAmount"].Value
		return msgbox('You must enter a valid number of eggs!', 'Error', 0x40010)
	StickerPrinter.getPos(&sx,&sy)
	StickerPrinter.move(sx,sy,156,139)
	PrinterAmount%i% := ((PrinterOption) ? "Infinite" : StickerPrinter["PrinterAmount"].Value || 1)
	IniWrite PrinterAmount%i%, "settings\nm_config.ini", "Collect", "PrinterAmount" i
	PrinterItem%i% := StickerPrinterEgg
	IniWrite PrinterItem%i%, "settings\nm_config.ini", "Collect", "PrinterItem" i
	PrinterTimer%i% := (StickerPrinter["PrinterTimer"].Text="Manual" ? StickerPrinter["PrinterTime"].Value * 600 : 43200)
	...
	StickerPrinter["PrinterTimer" i].Text := formathm(PrinterTimer%i%)
	for v in ["PrinterOption", "PrinterInt", "PrinterAmount", "PrinterAddSlot", "StickerPrinterEgg", "SPELeft", "SPERight", "OptLeft", "OptRight", "PrinterTimer", "PrinterTimeEdit", "PrinterTime", "PrinterHelp"]
	...
	loop 2 {
		...
		StickerPrinter["PrinterTimer" A_Index].Visible := 1
	}
}
nm_BoostChaserCheck(*){
...
nm_BoostedFieldSelectButton(*){
	...
	BoostedFieldSelectGui.Add("Text", "x10 y130", "Other")
...
nm_createPresetFiles(presetName, *) {
	...
	, BoostSettings := [
		...
		"PineappleBoosterCheck", "PepperBoosterCheck", "PumpkinBoosterCheck", "RoseBoosterCheck",
		"SpiderBoosterCheck", "StickerStackCheck", "StickerStackCub",
		"StickerStackHive", "StickerStackItem", "StickerStackMode",
		"StickerStackTimer", "StrawberryBoosterCheck", "StumpBoosterCheck", "SunflowerBoosterCheck"
	]
	, CollectSettings := [
	...
	for k in PresetSettingsCtrls {
		...
		if k = "KillTimers" or k = "PlantersTimers" or k = "BoostTimers" or k = "CollectTimers" {
			for v in %k% {
				if !configObj[section].Has(v)
					throw Error("Setting " v " not found in nm_config.ini")
...
nm_MeteorShower() {
	...
	updateConfig()
	updateConfig() {
		LastMeteorShower:=nowUnix()
		IniWrite LastMeteorShower, "settings\nm_config.ini", "Collect", "LastMeteorShower"
	}
}
...
nm_StickerPrinter(){
	global PrinterItem1, PrinterItem2, LastStickerPrinter, PrinterAmount1, PrinterAmount2, PrinterDonated1, PrinterDonated2, PrinterTimer1, PrinterTimer2, PrinterIndex
	t := PrinterIndex
	If (PrinterItem%t%!="None" && (PrinterAmount%t%="Infinite" ? true : PrinterDonated%t%<PrinterAmount%t%) && (nowUnix()-LastStickerPrinter)>PrinterTimer%t%) {
		loop 2 {
			hwnd := GetRobloxHWND()
			offsetY := GetYOffset(hwnd)
			GetRobloxClientPos(hwnd)
			...
			If (searchRet[1] = 0) {
				...
				if (Gdip_ImageSearch(pBMScreen, bitmaps["stickerprinterConfirm"], , , , , , 10) != 1) {
					Gdip_DisposeImage(pBMScreen)
					nm_setStatus("Error", "No Eggs left in inventory!`nSticker Printer Slot " t " has been disabled.")
					PrinterDonated%t% := PrinterAmount%t%
					Sleep 500
					sendinput "{" SC_E " down}"
					Sleep 100
					sendinput "{" SC_E " up}"
				...
				nm_setStatus("Collected", "Sticker Printer (" PrinterItem%t% " Egg)")
				LastStickerPrinter:=nowUnix()
				IniWrite LastStickerPrinter, "settings\nm_config.ini", "Collect", "LastStickerPrinter"
				++PrinterDonated%t%
				IniWrite PrinterDonated%t%, "settings\nm_config.ini", "Collect", "PrinterDonated" t
				PrinterIndex:=(PrinterIndex=1 ? 2 : 1)
				IniWrite PrinterIndex%t%, "settings\nm_config.ini", "Collect", "PrinterIndex" t
				break
			}
		}
	}
...
nm_toAnyBooster(){	
	...
	while((FieldBooster%A_Index%="none" && QuestBlueBoost=0 && QuestRedBoost=0) && A_Index < 4){
		LastBooster:=max(LastBlueBoost, LastRedBoost, LastMountainBoost, (BoostChaserCheck && CoconutBoosterCheck && CoconutDisCheck) ? LastCoconutDis : 1)

		if((FieldBooster%A_Index%="blue" && (nowUnix()-LastBlueBoost)>2700 && (nowUnix()-LastBooster)>(FieldBoosterMins*60)) || (QuestBlueBoost && (nowUnix()-LastBlueBoost)>3600)) ;Blue Field Booster
			nm_updateAction("Booster"), nm_toBooster("blue")

		else if((FieldBooster%A_Index%="red" && (nowUnix()-LastRedBoost)>2700 && (nowUnix()-LastBooster)>(FieldBoosterMins*60)) || (QuestRedBoost && (nowUnix()-LastRedBoost)>3600)) ;Red Field Booster
			nm_updateAction("Booster"), nm_toBooster("red")

		else if(FieldBooster%A_Index%="mountain" && (nowUnix()-LastMountainBoost)>2700 && (nowUnix()-LastBooster)>(FieldBoosterMins*60)) ;Mountain Top Field Booster
			nm_updateAction("Booster"), nm_toBooster("mountain")
	}
}
nm_toBooster(location){
	...
	Loop 2 {
		nm_Reset(0)
		nm_setStatus("Traveling", (location="coconut") ? "Coconut Field Booster" : (location="red") ? "Red Field Booster" : (location="blue") ? "Blue Field Booster" : "Mountain Top Booster")
...
WM_LBUTTONDOWN(wParam, lParam, msg, hwnd) {
	...
	switch mgui[ctrl].name, 0 {
		...
		case "roll":PostMessage(0x0112, 0xF060), blc_roll()
		case "selectAll":
...
blc_roll() {
	selectedBees := [], selectedMutations := 0
	for i in beeArr
		if %i% || SelectAll
			selectedBees.push(i)
	if mutations {
	...
	msgbox selectedBees.length " " (selectedMutations ? selectedMutations.length : 0)
	startGUI()
}
closeFunction(*) {
	global xPos, yPos
