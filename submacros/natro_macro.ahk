...
MainGui.Add("Button", "x305 yp+19 w80 h16 vStickerPrinter Disabled", "Sticker Printer").OnEvent("Click", nm_StickerPrinterGui)
...
nm_TabBoostLock(){
	...
	MainGui["StickerPrinter"].Enable := 0
}
nm_TabBoostUnLock(){
	global
...
nm_StickerPrinterGui(*) {
	...
	StickerPrinter.Add("UpDown", "vPrinterAmount range1-999 Disabled Hidden", 1)
...
nm_setPrinterData(GuiCtrl, *){
	...
	if (PrinterItem%i% = "None") {
		...
		StickerPrinter["PrinterTime"].Value := 1
...
nm_AddPrinterItem(*) {
	...
	PrinterAmount%i% := ((PrinterOption) ? "Infinite" : StickerPrinter["PrinterAmount"].Value)
	IniWrite PrinterAmount%i%, "settings\nm_config.ini", "Collect", "PrinterAmount" i
	PrinterItem%i% := StickerPrinterEgg
	IniWrite PrinterItem%i%, "settings\nm_config.ini", "Collect", "PrinterItem" i
	PrinterTimer%i% := (StickerPrinter["PrinterTimer"].Text="Manual" ? StickerPrinter["PrinterTime"].Value * 3600 : 43200)
...
nm_createPresetFiles(presetName, *) {
	static KillSettings := [
		...
		, CollectSettings := [
			...
			"JellyBeanMatchIgnore", "LidArtCheck", "MagicBeanMatchIgnore",
			"MegaMemoryMatchCheck", "MeteorShowerCheck", "MicroConverterMatchIgnore",
			"MondoAction", "MondoSecs", "MoonCharmMatchIgnore",
			"NeonberryMatchIgnore", "NightBellMatchIgnore", "NightLastDetected",
			"NightMemoryMatchCheck", "NormalMemoryMatchCheck", "OilMatchIgnore",
			"PineappleMatchIgnore", "RBPDelevelCheck", "RedExtractMatchIgnore",
			"RoboPassCheck", "RoyalJellyDisCheck", "RoyalJellyMatchIgnore",
			"SamovarCheck", "SilverEggMatchIgnore", "SmoothDiceMatchIgnore",
			"SoftWaxMatchIgnore", "StarJellyMatchIgnore", "StingerMatchIgnore",
			"StockingsCheck", "StrawberryDisCheck", "StrawberryMatchIgnore",
			"SunflowerSeedMatchIgnore", "SuperSmoothieMatchIgnore",
			"SwirledWaxMatchIgnore", "TicketMatchIgnore", "TreatDisCheck",
			"TreatMatchIgnore", "TropicalDrinkMatchIgnore", "WinterMemoryMatchCheck",
			"WreathCheck"
		]
		, PresetSettingsCtrls := [
