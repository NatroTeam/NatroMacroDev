...
nm_status(status)
{
	...
	if (discordCheck = 1)
	{
		...
		if ((ssCheck = 1)
			...
			|| ((state = "Gathering") && !InStr(objective, "Ended") && (HoneyUpdateCheck) && (pBM := CreateHoneyBitmap(1, 0)))
			|| ((state = "Converting") && (objective = "Backpack") && (HoneyUpdateCheck) && (pBM := CreateHoneyBitmap()))))
		{
			if !IsSet(pBM)
