nm_OCRFromBitmap(bitmap, withRegions := false) {
	static language := ""
	if !language {
		...
		if !language
			throw Error("Install an English Windows OCR language to read inventory items.")
	}
	hBitmap := Gdip_CreateHBITMAPFromBitmap(bitmap)
	try return nm_OCRRead(nm_OCRBitmapStream(hBitmap), language, withRegions)
...
nm_OCRRead(input, language := "", withRegions := false) {
