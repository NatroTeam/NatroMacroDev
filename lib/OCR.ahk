...
class RapidOcr {
	...
	static __cb(i) {
		static cbs := [{ ptr: CallbackCreate(get_text), __Delete: this => CallbackFree(this.ptr) }, { ptr: CallbackCreate(get_result), __Delete: this => CallbackFree(this.ptr) },
		]
		return cbs[i]
	...
	ocr_from_pbitmap(pBitmap, param := 0, allresult := false) {
		Gdip_GetImageDimensions(pBitmap, &width, &height)
		Gdip_LockBits(pBitmap, 0,0,width,height, &stride, &scan0,&bitmapData, 1)
		data := ClipboardAll(scan0, stride * height)
		NumPut("ptr", data.ptr, "uint", stride, "uint", width, "uint", height, "uint", 4, "uint", 0, "uint", 0, info := Buffer(40, 0))
		Gdip_UnlockBits(pBitmap, &bitmapData)
		return this.ocr_from_bitmapdata(info, param, allresult)
	}
	class OcrParam extends Buffer {
		static Default() {
			...
			self.doAngle := false
			self.boxThresh := 0.1
			self.unClipRatio := 1.9
			self.padding := 0
			self.boxScoreThresh := 0.1
			return self
		}
		__New(param?) {
			super.__New(42, 0)
