DllCall(DllCall("GetProcAddress"
    ...
    , "AStr", "USkinInit", "Ptr")
    , "Int", 0, "Int", 0, "AStr", A_WorkingDir "\nm_image_assets\styles\core.msstyles")
...
nm_stickerPrinterGUI() {
    ...
    printerEggIndex := 1, AddEggIndex := 0
    (StickerPrinterGUI := Gui(, "Sticker Printer")).Show("w190 h120")
    xp := 0
    loop 3 {
        StickerPrinterGUI.Add("Button", "x" (xp += 5) " y90 w30 h20 Hidden vAddButton" A_Index, "+").OnEvent("Click", IncreaseAmount)
        ...
        StickerPrinterGUI.Add("Button", "x" (xp += 30) " y90 w30 h20 Hidden vRemoveButton" A_Index, "-").OnEvent("Click", DecreaseAmount), xp += 30
    }
    ...
    StickerPrinterGUI.Add("GroupBox", "x106 y-3 w90 h66 vamountGB Hidden")
    StickerPrinterGUI.Add("GroupBox", "x106 yp+64 w90 hp vPrinterItems Hidden")
    StickerPrinterGUI.Add("CheckBox", "x110 y5 w80 h20 vInfiniteCheck Hidden", "Infinite")
    switchMode 0
    switchMode(mode := 0) {
        static controls0 := ["AddButton1", "AddButton2", "AddButton3", "RemoveButton1", "RemoveButton2", "RemoveButton3", "AddItemButton1", "AddItemButton2", "AddItemButton3", "ItemCount1", "ItemCount2", "ItemCount3", "PrinterTimer1", "PrinterTimer2", "PrinterTimer3", "StickerPrinterEgg1", "StickerPrinterEgg2", "StickerPrinterEgg3"],
            controls1 := ["BackButton", "StickerPrinterEggAdd", "AddEggLeft", "AddEggRight", "AddEggButton", "Discriminator", "amountGB", "InfiniteCheck", "PrinterItems"]
        switch mode {
            case 0:
                for i in controls1
                ...
                loop 3 {
                    if PrinterItem%A_Index% {
                        ...
                        if PrinterAmount%A_Index% = "infinite"
                            StickerPrinterGUI["AddButton" A_Index].enabled := false
    ...
    AddItem(control, *) {
        AddEggIndex := SubStr(control.name, -1)
        switchMode 1
    }
}
nm_stickerPrinterGUI()
